...
import { 
  ...
  Palette, KeyRound, Menu, Users, ShieldAlert, DownloadCloud
} from 'lucide-react';
import { 
...
const App: React.FC = () => {
  ...
  const handleArchiveMonth = () => {
    if (expenses.length === 0 && salary === 0) {
      alert('لا توجد بيانات لترحيلها');
      return;
    }
    const confirmArchive = window.confirm('هل أنت متأكد من ترحيل بيانات هذا الشهر للأرشيف؟ سيتم تصفير القائمة الحالية.');
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 font-['Cairo']">
        <div className="bg-white w-full max-w-[440px] rounded-[2.5rem] p-8 md:p-12 text-center shadow-2xl animate-in zoom-in-95">
          <div className="inline-flex bg-blue-600 w-20 h-20 rounded-[1.5rem] items-center justify-center mb-8 shadow-xl">
            <Wallet className="text-white" size={40} />
          </div>
          <h1 className="text-3xl font-black text-slate-900 mb-2">Smart Prise</h1>
          <p className="text-slate-500 mb-8">أهلاً بك في نظام الإدارة المالية</p>
          <form onSubmit={handleLogin} className="space-y-4 text-right">
            <div>
              <label className="text-[10px] font-black text-slate-400 mr-2 uppercase tracking-widest">اسم المستخدم</label>
              <input className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold outline-none focus:ring-2 focus:ring-blue-500" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} required placeholder="أدخل اسم المستخدم" />
            </div>
            <div className="relative">
              <label className="text-[10px] font-black text-slate-400 mr-2 uppercase tracking-widest">كلمة المرور</label>
              <input type={showPassword ? "text" : "password"} className="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 font-bold outline-none focus:ring-2 focus:ring-blue-500" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} required placeholder="••••••••" />
            ...
            <button className="w-full bg-slate-900 text-white font-black py-4 rounded-xl hover:bg-blue-600 transition-all shadow-lg active:scale-95">دخول للنظام</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className={`flex h-screen overflow-hidden transition-colors duration-300 font-['Cairo'] ${themeClasses[theme]}`}>
      ...
      <aside className={`w-72 flex-col shadow-xl z-20 border-l border-white/5 bg-slate-900 text-white hidden lg:flex`}>
        ...
        <nav className="flex-1 p-6 space-y-2">
          {[
            ...
            { id: AppTab.PREVIOUS_MONTHS, icon: Archive, label: 'الأشهر السابقة' },
        ...
        <div className="p-8 border-t border-white/5 space-y-4">
          ...
          <button onClick={() => setCurrentUser(null)} className="text-red-400 font-bold flex items-center gap-2 pt-4 border-t border-white/5 hover:text-red-300"><LogOut size={16}/> تسجيل الخروج</button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className={`p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/5 ${cardClasses[theme]}`}>
          <div className="flex items-center gap-4 w-full md:w-auto">
             <button className="lg:hidden p-3 bg-white/5 rounded-xl"><Menu size={24}/></button>
             <div>
          ...
          <div className="flex items-center gap-12 w-full md:w-auto justify-between md:justify-end">
            <div className="text-center">
              <p className="text-[10px] font-black opacity-40 mb-1">إجمالي المصروفات</p>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 pb-32">
          {activeTab === AppTab.MONTHLY_EXPENSES && (
            <div className="animate-in slide-in-from-bottom-5 duration-500">
              ...
              <div className="grid gap-4">
                {expenses.length === 0 ? (
                  <div className="py-20 text-center opacity-20"><TrendingDown size={64} className="mx-auto mb-4" /><p className="font-bold">لا توجد مصروفات مسجلة لهذا الشهر</p></div>
                ) : (
                  expenses.map(exp => (
                      ...
                      <div className="flex items-center gap-6">
                        ...
                        <div className="flex gap-2">
                           <button onClick={() => {
                             ...
                             setExpenses(updated);
                             syncFinancialData({ expenses: updated });
                           }} className={`p-2 rounded-lg ${exp.isPaid ? 'text-blue-400' : 'text-green-500 hover:bg-green-500/10'}`}><CheckCircle2 size={24}/></button>
                           <button onClick={() => {
                             const updated = expenses.filter(e => e.id !== exp.id);
                             setExpenses(updated);
                             syncFinancialData({ expenses: updated });
                           }} className="p-2 text-red-500 hover:bg-red-500/10 rounded-lg"><Trash2 size={24}/></button>
                        </div>
                      </div>
                    </div>
                  ))
                  ...
                  <div key={com.id} className={`p-8 rounded-[2rem] border relative overflow-hidden flex flex-col justify-between h-full ${cardClasses[theme]}`}>
                    <div>
                      <div className="flex justify-between mb-4">
                        ...
                        <div className="flex gap-2">
                          {currentUser.isAdmin && (
                            <button onClick={() => setIsAdminPassModalOpen(true)} className="p-2 text-blue-400 hover:bg-blue-400/10 rounded-lg"><Edit3 size={18}/></button>
                          )}
                          <button onClick={() => {
                    ...
                    <div>
                       <div className="flex justify-between text-xs font-bold mb-2">
                          ...
                          <span className="text-emerald-500">المتبقي: {com.remainingAmount.toLocaleString()}</span>
                       </div>
                       <div className="w-full bg-white/5 h-2.5 rounded-full overflow-hidden mb-4">
                          <div className="bg-emerald-500 h-full transition-all duration-700" style={{width: `${(com.paidAmount/com.totalValue)*100}%`}}></div>
                       </div>
                       <button onClick={() => {
                          const updated = commitments.map(c => {
                            if (c.id === com.id) {
                              const installment = c.totalValue / c.installmentsCount;
                              const newPaid = Math.min(c.paidAmount + installment, c.totalValue);
                              return {...c, paidAmount: newPaid, remainingAmount: c.totalValue - newPaid, isCompleted: newPaid >= c.totalValue};
                            }
                            return c;
                          });
                          setCommitments(updated);
                          syncFinancialData({ commitments: updated });
                       }} disabled={com.isCompleted} className={`w-full py-3 rounded-xl font-black transition-all ${com.isCompleted ? 'bg-slate-500/20 text-slate-500' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
          ...
          {activeTab === AppTab.PREVIOUS_MONTHS && (
            <div className="animate-in slide-in-from-bottom-5 duration-500">
               ...
               {history.length === 0 ? (
                 <div className="py-24 text-center opacity-20 border-2 border-dashed border-white/10 rounded-[2rem]"><Archive size={64} className="mx-auto mb-4"/><p className="font-bold text-xl">الأرشيف فارغ حالياً</p></div>
               ) : (
                     ...
                     <div key={record.id} className={`rounded-[2rem] border overflow-hidden ${cardClasses[theme]}`}>
                        ...
                        <div className="p-8 grid md:grid-cols-2 gap-8">
                           ...
                           <div className="flex flex-col justify-center bg-white/5 p-6 rounded-2xl text-center">
                              ...
                              <p className="text-[10px] opacity-40 font-black uppercase">من راتب أصلي {record.salary.toLocaleString()}</p>
                           </div>
                        </div>
                     </div>
                   ))}
          ...
          {activeTab === AppTab.SETTINGS && (
            <div className="animate-in slide-in-from-bottom-5 duration-500">
               <h2 className="text-2xl font-black mb-10 text-right">إعدادات النظام</h2>
               <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className={`p-8 rounded-[2rem] border ${cardClasses[theme]}`}>
                    <h3 className="font-black text-xl mb-6 flex items-center gap-2 text-blue-500"><UserPlus size={24}/> الحسابات النشطة</h3>
                    <div className="space-y-4">
                       {users.map(u => (
                         <div key={u.id} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/5">
                            <div className="flex items-center gap-3">
                               <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center font-black text-white">{u.username.charAt(0).toUpperCase()}</div>
                               <div><p className="font-black">{u.username}</p><p className="text-[10px] opacity-40 uppercase">{u.isAdmin ? 'مدير' : 'مستخدم'}</p></div>
                            </div>
                         </div>
                       ))}
                    </div>
            ...
            <div className={`w-[calc(100vw-2rem)] sm:w-96 h-[550px] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden border animate-in slide-in-from-bottom-10 ${cardClasses[theme]}`}>
              <div className="bg-blue-600 p-6 text-white flex justify-between items-center">
                <div className="flex items-center gap-3 font-black">
                   <div className="w-8 h-8 rounded-lg bg-white/20 flex items-center justify-center"><Users size={18}/></div>
                   <div>
                      <p className="text-sm">{chatRecipient === 'all' ? 'المحادثة العامة' : `خاص: ${users.find(u => u.id === chatRecipient)?.username}`}</p>
                      <p className="text-[9px] opacity-60 uppercase">{chatRecipient === 'all' ? 'الكل يشارك هنا' : 'محادثة سرية'}</p>
                   </div>
                </div>
              ...
              <div className="p-3 bg-white/5 border-b border-white/5 flex gap-2 overflow-x-auto scrollbar-hide">
                <button onClick={() => setChatRecipient('all')} className={`px-4 py-2 rounded-full text-[10px] font-black transition-all ${chatRecipient === 'all' ? 'bg-blue-600 text-white' : 'bg-white/10 opacity-50'}`}>المجموعة</button>
                {users.filter(u => u.id !== currentUser.id).map(u => (
                  <button key={u.id} onClick={() => setChatRecipient(u.id)} className={`px-4 py-2 rounded-full text-[10px] font-black transition-all ${chatRecipient === u.id ? 'bg-blue-600 text-white' : 'bg-white/10 opacity-50'}`}>{u.username}</button>
                ))}
              </div>
                    ...
                    <div key={msg.id} className={`flex flex-col group ${msg.fromUserId === currentUser.id ? 'items-end' : 'items-start'}`}>
                      <div className="flex items-center gap-2 mb-1 px-2">
                        ...
                        {msg.fromUserId === currentUser.id && (
                          <button onClick={() => deleteMessage(msg.id)} className="opacity-0 group-hover:opacity-100 text-red-500 transition-all hover:scale-110"><Trash2 size={10}/></button>
                        )}
                      </div>
                      <div className={`p-4 rounded-2xl max-w-[85%] text-[13px] font-bold shadow-sm leading-relaxed ${msg.fromUserId === currentUser.id ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white/10 text-inherit rounded-tl-none'}`}>
                        {msg.text}
                        <p className="text-[8px] mt-2 opacity-40 text-left border-t border-white/10 pt-1">{msg.timestamp}</p>
                      </div>
                    </div>
                  ))
                )}
              ...
              <div className="p-4 bg-white/5 border-t border-white/5">
                <div className="flex gap-2 mb-3 overflow-x-auto pb-1 scrollbar-hide">
                  {EMOJIS.map(e => <button key={e} onClick={() => setChatInput(p => p + e)} className="p-1 hover:bg-white/10 rounded text-xl active:scale-125 transition-all">{e}</button>)}
                </div>
                <div className="flex gap-2">
      ...
      {isExpenseModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
            ...
            <div className="space-y-6">
              <div className="flex gap-3">
                <div className="flex-1">
                  <label className="text-[10px] font-black opacity-40 uppercase mr-2">الفئة</label>
                  {!isAddingCustomCategory ? (
                    <select className="w-full bg-white/5 p-4 rounded-xl border border-white/10 outline-none font-bold" value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)}>
                      {DEFAULT_EXPENSE_CATEGORIES.map(c => <option key={c} value={c} className="bg-slate-900">{c}</option>)}
                    </select>
                  ) : (
                    <input className="w-full bg-white/5 p-4 rounded-xl border border-white/10 outline-none font-bold" placeholder="فئة مخصصة" value={tempExpense.name} onChange={e => setTempExpense({...tempExpense, name: e.target.value})} />
                  )}
                </div>
                <button onClick={() => setIsAddingCustomCategory(!isAddingCustomCategory)} className="mt-6 bg-white/10 p-4 rounded-xl hover:bg-white/20 transition-all text-blue-500"><Plus size={24}/></button>
              </div>
              <div>
                <label className="text-[10px] font-black opacity-40 uppercase mr-2">القيمة (ج.م)</label>
                <input type="number" placeholder="0.00" className="w-full bg-white/5 p-5 rounded-xl border border-white/10 outline-none font-black text-3xl text-blue-500" value={tempExpense.value} onChange={e => setTempExpense({...tempExpense, value: e.target.value})} />
              </div>
              <div>
                 <label className="text-[10px] font-black opacity-40 uppercase mr-2">موعد السداد</label>
              ...
              <div className="flex gap-4 pt-6">
                <button onClick={handleAddExpense} className="flex-1 bg-blue-600 text-white py-5 rounded-2xl font-black shadow-lg">تأكيد الإضافة</button>
                <button onClick={() => setIsExpenseModalOpen(false)} className="flex-1 bg-white/10 py-5 rounded-2xl font-black hover:bg-white/20">إلغاء</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {isCommitmentModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className={`w-full max-w-lg rounded-[2.5rem] p-10 border shadow-2xl ${cardClasses[theme]}`}>
            <h3 className="text-2xl font-black mb-8">إضافة التزام مالي</h3>
            <div className="space-y-4">
               <div>
                  <label className="text-[10px] font-black opacity-40 uppercase mr-2">نوع الالتزام</label>
                  <select className="w-full bg-white/5 p-4 rounded-xl border border-white/10 font-bold outline-none" value={tempCommitment.type} onChange={e => setTempCommitment({...tempCommitment, type: e.target.value})}>
                    {COMMITMENT_TYPES.map(c => <option key={c} value={c} className="bg-slate-900">{c}</option>)}
                  </select>
               </div>
               <div>
                  <label className="text-[10px] font-black opacity-40 uppercase mr-2">المبلغ الإجمالي</label>
                  <input type="number" className="w-full bg-white/5 p-4 rounded-xl border border-white/10 font-black text-xl" value={tempCommitment.totalValue} onChange={e => setTempCommitment({...tempCommitment, totalValue: e.target.value})} />
               </div>
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black opacity-40 uppercase mr-2">عدد الأقساط</label>
                    <input type="number" className="w-full bg-white/5 p-4 rounded-xl border border-white/10 font-bold" value={tempCommitment.installmentsCount} onChange={e => setTempCommitment({...tempCommitment, installmentsCount: e.target.value})} />
                  </div>
                  <div>
                    <label className="text-[10px] font-black opacity-40 uppercase mr-2">المدة (أشهر)</label>
                    <input placeholder="12 شهر" className="w-full bg-white/5 p-4 rounded-xl border border-white/10 font-bold" value={tempCommitment.duration} onChange={e => setTempCommitment({...tempCommitment, duration: e.target.value})} />
                  </div>
               </div>
               <div className="flex gap-4 pt-6">
                 <button onClick={() => {
                   ...
                   const newCom = {
                     id: Date.now().toString(),
                     type: tempCommitment.type || COMMITMENT_TYPES[0],
                     totalValue: total,
                 ...
                 }} className="flex-1 bg-emerald-600 text-white py-5 rounded-2xl font-black">إضافة الالتزام</button>
                 <button onClick={() => setIsCommitmentModalOpen(false)} className="flex-1 bg-white/10 py-5 rounded-2xl font-black hover:bg-white/20">إلغاء</button>
               </div>
            </div>
          </div>
        </div>
      ...
      {isAdminPassModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
           <div className={`w-full max-w-md rounded-[2rem] p-10 border shadow-2xl ${cardClasses[theme]}`}>
              <h3 className="text-xl font-black mb-6 flex items-center gap-2"><KeyRound className="text-blue-500"/> تعديل كلمة سر المدير</h3>
              <div className="space-y-4">
                 <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="كلمة المرور الجديدة" className="w-full bg-white/5 p-4 rounded-xl border border-white/5 font-bold outline-none focus:ring-1" />
                 <div className="flex gap-3">
                    <button onClick={() => {
                    ...
                    }} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-black">حفظ التغيير</button>
                    <button onClick={() => setIsAdminPassModalOpen(false)} className="flex-1 bg-white/10 py-3 rounded-xl font-black">إلغاء</button>
                 </div>
              </div>
           </div>
        </div>
